...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!foundProperty)
      return res.status(HTTP_NOT_FOUND).json({ success: false, message: 'Property not found.' });
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    if (req.user?.isAdmin)
      return res.status(HTTP_FORBIDDEN).json({ success: false, message: 'Admin cannot add properties.' });

    // Tenant role is also blocked — must be landlord
    if (req.user.role !== 'landlord')
      return res.status(HTTP_FORBIDDEN).json({ success: false, message: 'Only landlords can add properties.' });
    ...
    res.status(HTTP_CREATED).json({ success: true, property: createdProperty });
...
router.put('/:id', protect, async (req, res) => {
  try {
    ...
    if (!req.user?.isAdmin && !callerIsOwner)
      return res.status(HTTP_FORBIDDEN).json({ success: false, message: 'Not authorized.' });
