...
public class TMSWhiteBoxTests {
    ...
    static void run(Object obj) {
        ...
        for (Method m : methods) {
            ...
            try {
                ...
                System.out.printf("  ❌ %s%n     ↳ %s%n", name, cause.getMessage());
                failed++;
                failures.add(name + "\n       " + cause.getMessage());
            } catch (IllegalAccessException e) {
                System.out.printf("  ❌ %s%n     ↳ Access error: %s%n", name, e.getMessage());
                failed++;
            }
        }
    }
    ...
    static Map<String, Object> propFilter(Map<String, String> q) {
        ...
        if (q.containsKey("beds"))    { try { f.put("beds_gte",  Integer.parseInt(q.get("beds"))); }   catch (NumberFormatException e) { f.put("beds_error", "Invalid"); } }
        if (q.containsKey("maxRent")) { try { f.put("rent_lte",  Double.parseDouble(q.get("maxRent"))); } catch (NumberFormatException e) { f.put("rent_error", "Invalid"); } }
        return f;
    }
    ...
    static class AuthTests {
        ...
        void t008() {
            ...
            assertFalse((Boolean)r.get("success"), "Role superuser should be rejected");
        }
        ...
        void t019() {
            var r = authLogin("user@g.com", "wrong", "correct", "active");
        ...
        void t020() {
            ...
            assertFalse((Boolean)r.get("success"), "Blocked account should fail");
        ...
        void t021() {
            ...
            assertTrue((Boolean)r.get("success"), "Valid login should succeed");
            assertEquals(200, r.get("status"));
        }
        @Test @DisplayName("TC-AUTH-WB-022: Blocked user with correct password still denied")
        void t022() {
            var r = authLogin("user@g.com", "pass123", "pass123", "blocked");
            assertFalse((Boolean)r.get("success"), "Blocked + correct password = still denied");
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-AUTH-WB-023: Valid JWT token - exactly 3 dot-separated parts")
        void t023() {
            assertTrue(validJWT("a.b.c"), "3-part token must be valid");
        }
        @Test @DisplayName("TC-AUTH-WB-024: Invalid JWT - only 2 parts")
        void t024() {
            assertFalse(validJWT("header.payload"), "2-part token must be invalid");
        }
        @Test @DisplayName("TC-AUTH-WB-025: Invalid JWT - null")
        void t025() {
            assertFalse(validJWT(null), "Null token must be invalid");
        }
        @Test @DisplayName("TC-AUTH-WB-026: Invalid JWT - empty string")
        void t026() {
            assertFalse(validJWT(""), "Empty token must be invalid");
        }
        @Test @DisplayName("TC-AUTH-WB-027: Invalid JWT - 4 parts (too many segments)")
        void t027() {
            assertFalse(validJWT("a.b.c.d"), "4-part token must be invalid");
        }
    }
    ...
    static class PropertyTests {
        ...
        void t001() {
            var r = propCreate("landlord", false, "3 Bed Flat", 25000);
            assertTrue((Boolean)r.get("success"), "Landlord create should pass");
            assertEquals(201, r.get("status"));
        }
        @Test @DisplayName("TC-PROP-WB-002: Admin cannot create property → 403")
        void t002() {
            var r = propCreate("admin", true, "Flat", 10000);
            assertFalse((Boolean)r.get("success"), "Admin create must fail");
            assertEquals("Admin cannot add properties.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-PROP-WB-003: Tenant cannot create property → 403")
        void t003() {
            var r = propCreate("tenant", false, "Flat", 10000);
            assertFalse((Boolean)r.get("success"), "Tenant create must fail");
            assertEquals("Only landlords can add properties.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-PROP-WB-004: Create fails - blank title")
        void t004() {
            var r = propCreate("landlord", false, "", 20000);
            assertFalse((Boolean)r.get("success"), "Blank title must fail");
            assertEquals("Property title required.", r.get("message"));
        }
        @Test @DisplayName("TC-PROP-WB-005: Create fails - null title")
        void t005() {
            var r = propCreate("landlord", false, null, 20000);
            assertFalse((Boolean)r.get("success"), "Null title must fail");
        }
        @Test @DisplayName("TC-PROP-WB-006: Create fails - missing rent")
        void t006() {
            var r = propCreate("landlord", false, "Nice Flat", null);
            assertFalse((Boolean)r.get("success"), "Missing rent must fail");
            assertEquals("Rent amount required.", r.get("message"));
        }
        @Test @DisplayName("TC-PROP-WB-007: Owner can update own property")
        void t007() {
            var r = propAuth("user123", "user123", false);
            assertTrue((Boolean)r.get("success"), "Owner update must pass");
        }
        @Test @DisplayName("TC-PROP-WB-008: Admin can update any property")
        void t008() {
            var r = propAuth("admin", "user123", true);
            assertTrue((Boolean)r.get("success"), "Admin update must pass");
        }
        @Test @DisplayName("TC-PROP-WB-009: Non-owner cannot update property → 403")
        void t009() {
            var r = propAuth("user456", "user123", false);
            assertFalse((Boolean)r.get("success"), "Non-owner update must fail");
            assertEquals("Not authorized.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-PROP-WB-010: Owner can delete own property")
        void t010() {
            var r = propAuth("user123", "user123", false);
            assertTrue((Boolean)r.get("success"), "Owner delete must pass");
        }
        @Test @DisplayName("TC-PROP-WB-011: Admin can delete any property")
        void t011() {
            var r = propAuth("admin", "anyUser", true);
            assertTrue((Boolean)r.get("success"), "Admin delete must pass");
        }
        @Test @DisplayName("TC-PROP-WB-012: Non-owner cannot delete property → 403")
        void t012() {
            var r = propAuth("tenant99", "landlord77", false);
            assertFalse((Boolean)r.get("success"), "Non-owner delete must fail");
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-PROP-WB-013: Filter by status 'available'")
        void t013() {
            var q = Map.of("status", "available");
            assertEquals("available", propFilter(q).get("status"));
        }
        @Test @DisplayName("TC-PROP-WB-014: Filter by area 'DHA Lahore'")
        void t014() {
            var q = Map.of("area", "DHA Lahore");
            assertEquals("DHA Lahore", propFilter(q).get("area"));
        }
        @Test @DisplayName("TC-PROP-WB-015: Filter by maxRent 30000 → stored as double")
        void t015() {
            var q = Map.of("maxRent", "30000");
            assertEquals(30000.0, propFilter(q).get("rent_lte"));
        }
        @Test @DisplayName("TC-PROP-WB-016: Filter by min beds = 3")
        void t016() {
            var q = Map.of("beds", "3");
            assertEquals(3, propFilter(q).get("beds_gte"));
        }
        @Test @DisplayName("TC-PROP-WB-017: Filter invalid beds 'abc' → error key set")
        void t017() {
            var q = Map.of("beds", "abc");
            assertNotNull(propFilter(q).get("beds_error"), "Invalid beds must produce error");
        }
        @Test @DisplayName("TC-PROP-WB-018: Filter invalid maxRent 'xyz' → error key set")
        void t018() {
            var q = Map.of("maxRent", "xyz");
            assertNotNull(propFilter(q).get("rent_error"), "Invalid maxRent must produce error");
        }
        @Test @DisplayName("TC-PROP-WB-019: Empty filter query → empty filter map")
        void t019() {
            assertTrue(propFilter(new HashMap<>()).isEmpty(), "Empty query → empty filter");
        }
        @Test @DisplayName("TC-PROP-WB-020: Multiple filters combined correctly")
        void t020() {
            Map<String, String> q = new HashMap<>();
            q.put("status", "available"); q.put("area", "Gulberg");
            q.put("maxRent", "50000"); q.put("beds", "2");
            var f = propFilter(q);
            assertEquals("available", f.get("status"));
            assertEquals("Gulberg",   f.get("area"));
            assertEquals(50000.0,     f.get("rent_lte"));
            assertEquals(2,           f.get("beds_gte"));
        }
    }
    ...
    static class PaymentTests {
        ...
        void t001() {
            var r = payCreate(false, 25000.0, "March 2025");
            assertTrue((Boolean)r.get("success"), "Tenant payment must pass");
            assertEquals(201, r.get("status"));
            assertEquals("paid", r.get("paymentStatus"));
        }
        @Test @DisplayName("TC-PAY-WB-002: Admin cannot add payment → 403")
        void t002() {
            var r = payCreate(true, 10000.0, "March 2025");
            assertFalse((Boolean)r.get("success"), "Admin payment must fail");
            assertEquals("Admin cannot add payments.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-PAY-WB-003: Payment fails - null amount")
        void t003() {
            var r = payCreate(false, null, "March 2025");
            assertFalse((Boolean)r.get("success"), "Null amount must fail");
            assertEquals("Amount and month required.", r.get("message"));
        }
        @Test @DisplayName("TC-PAY-WB-004: Payment fails - empty month")
        void t004() {
            var r = payCreate(false, 15000.0, "");
            assertFalse((Boolean)r.get("success"), "Empty month must fail");
            assertEquals("Amount and month required.", r.get("message"));
        }
        @Test @DisplayName("TC-PAY-WB-005: Payment fails - negative amount")
        void t005() {
            var r = payCreate(false, -500.0, "April 2025");
            assertFalse((Boolean)r.get("success"), "Negative amount must fail");
            assertEquals("Amount must be positive.", r.get("message"));
        }
        @Test @DisplayName("TC-PAY-WB-006: Payment fails - zero amount (boundary)")
        void t006() {
            var r = payCreate(false, 0.0, "April 2025");
            assertFalse((Boolean)r.get("success"), "Zero amount must fail");
        }
        @Test @DisplayName("TC-PAY-WB-007: Payment succeeds - amount = 1 (minimum boundary)")
        void t007() {
            var r = payCreate(false, 1.0, "April 2025");
            assertTrue((Boolean)r.get("success"), "Amount=1 must pass");
        }
        @Test @DisplayName("TC-PAY-WB-008: Admin sees ALL payments (filter=all)")
        void t008() { assertEquals("all", payFilter(true, "admin")); }
        @Test @DisplayName("TC-PAY-WB-009: Tenant sees only own payments (filter=tenantId)")
        void t009() { assertEquals("tenantId", payFilter(false, "tenant")); }
        @Test @DisplayName("TC-PAY-WB-010: Landlord sees own payments (filter=landlordId)")
        void t010() { assertEquals("landlordId", payFilter(false, "landlord")); }
    }
    ...
    static class LeaseTests {
        ...
        void t001() {
            var r = leaseCreate(false, "2025-01-01", "2025-12-31");
            assertTrue((Boolean)r.get("success"), "Tenant lease must pass");
            assertEquals("active", r.get("leaseStatus"));
        }
        @Test @DisplayName("TC-LEASE-WB-002: Admin cannot sign lease → 403")
        void t002() {
            var r = leaseCreate(true, "2025-01-01", "2025-12-31");
            assertFalse((Boolean)r.get("success"), "Admin lease must fail");
            assertEquals("Admin cannot sign leases.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-LEASE-WB-003: Lease fails - null startDate")
        void t003() {
            var r = leaseCreate(false, null, "2025-12-31");
            assertFalse((Boolean)r.get("success"), "Null startDate must fail");
            assertEquals("Start and end date required.", r.get("message"));
        }
        @Test @DisplayName("TC-LEASE-WB-004: Lease fails - empty endDate")
        void t004() {
            var r = leaseCreate(false, "2025-01-01", "");
            assertFalse((Boolean)r.get("success"), "Empty endDate must fail");
        }
        @Test @DisplayName("TC-LEASE-WB-005: Lease fails - both dates null")
        void t005() {
            var r = leaseCreate(false, null, null);
            assertFalse((Boolean)r.get("success"), "Both null dates must fail");
        }
        @Test @DisplayName("TC-LEASE-WB-006: Admin sees all leases (admin filter)")
        void t006() {
            // same filter logic as payments
            assertEquals("all", payFilter(true, "admin"));
        }
        @Test @DisplayName("TC-LEASE-WB-007: Tenant sees own leases")
        void t007() { assertEquals("tenantId", payFilter(false, "tenant")); }
    }
    ...
    static class MaintenanceTests {
        ...
        void t001() {
            var r = maintCreate(false, "Pipe leakage in bathroom");
            assertTrue((Boolean)r.get("success"), "Tenant maintenance must pass");
            assertEquals("pending", r.get("reqStatus"));
        }
        @Test @DisplayName("TC-MAINT-WB-002: Admin cannot submit maintenance → 403")
        void t002() {
            var r = maintCreate(true, "Test");
            assertFalse((Boolean)r.get("success"), "Admin submit must fail");
            assertEquals("Admin cannot submit maintenance.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-MAINT-WB-003: Request fails - empty description")
        void t003() {
            var r = maintCreate(false, "");
            assertFalse((Boolean)r.get("success"), "Empty description must fail");
            assertEquals("Description is required.", r.get("message"));
        }
        @Test @DisplayName("TC-MAINT-WB-004: Request fails - null description")
        void t004() {
            var r = maintCreate(false, null);
            assertFalse((Boolean)r.get("success"), "Null description must fail");
        }
        @Test @DisplayName("TC-MAINT-WB-005: Request fails - whitespace-only description")
        void t005() {
            var r = maintCreate(false, "   ");
            assertFalse((Boolean)r.get("success"), "Whitespace description must fail");
        }
        @Test @DisplayName("TC-MAINT-WB-006: Admin updates status to 'in-progress'")
        void t006() {
            var r = maintUpdateStatus(true, "in-progress");
            assertTrue((Boolean)r.get("success"), "Admin status update must pass");
        ...
        void t007() {
            ...
            assertTrue((Boolean)r.get("success"), "Resolve must pass");
        ...
        void t008() {
            ...
            assertFalse((Boolean)r.get("success"), "Tenant status update must fail");
            assertEquals("Admin access required.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
    }
    ...
    static class ComplaintsTests {
        ...
        void t001() {
            var r = compCreate(false, "Water supply issue");
            assertTrue((Boolean)r.get("success"), "Tenant complaint must pass");
            assertEquals("open", r.get("compStatus"));
        }
        @Test @DisplayName("TC-COMP-WB-002: Admin cannot file complaint → 403")
        void t002() {
            var r = compCreate(true, "Test subject");
            assertFalse((Boolean)r.get("success"), "Admin complaint must fail");
            assertEquals("Admin cannot file complaints.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-COMP-WB-003: Complaint fails - empty subject")
        void t003() {
            var r = compCreate(false, "");
            assertFalse((Boolean)r.get("success"), "Empty subject must fail");
            assertEquals("Subject is required.", r.get("message"));
        }
        @Test @DisplayName("TC-COMP-WB-004: Complaint fails - null subject")
        void t004() {
            var r = compCreate(false, null);
            assertFalse((Boolean)r.get("success"), "Null subject must fail");
        }
        @Test @DisplayName("TC-COMP-WB-005: Complaint fails - whitespace subject")
        void t005() {
            var r = compCreate(false, "   ");
            assertFalse((Boolean)r.get("success"), "Whitespace subject must fail");
        }
    ...
    static class UserAdminTests {
        ...
        void t001() {
            ...
            assertTrue((Boolean)r.get("success"), "Block must pass");
        ...
        void t002() {
            ...
            assertTrue((Boolean)r.get("success"), "Unblock must pass");
        ...
        void t003() {
            ...
            assertFalse((Boolean)r.get("success"), "Non-admin block must fail");
            assertEquals("Admin access required.", r.get("message"));
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-USER-WB-004: Toggle twice returns original state (idempotency)")
        void t004() {
            var first  = userToggleBlock(true, "active");   // → blocked
            var second = userToggleBlock(true, (String)first.get("newStatus")); // → active
            assertEquals("active", second.get("newStatus"), "Double-toggle must restore original");
        }
        @Test @DisplayName("TC-USER-WB-005: Toggle on null user → 404")
        void t005() {
            var r = userToggleBlock(true, null);
            assertFalse((Boolean)r.get("success"), "Null user toggle must fail");
            assertEquals(404, r.get("status"));
        }
        @Test @DisplayName("TC-USER-WB-006: Admin verifies user → verified=true")
        void t006() {
            var r = userVerify(true, "user123");
            assertTrue((Boolean)r.get("success"), "Verify must pass");
            assertTrue((Boolean)r.get("verified"), "verified flag must be true");
        }
        @Test @DisplayName("TC-USER-WB-007: Non-admin cannot verify user → 403")
        void t007() {
            var r = userVerify(false, "user123");
            assertFalse((Boolean)r.get("success"), "Non-admin verify must fail");
            assertEquals(403, r.get("status"));
        }
        @Test @DisplayName("TC-USER-WB-008: Verify fails - empty user ID")
        void t008() {
            var r = userVerify(true, "");
            assertFalse((Boolean)r.get("success"), "Empty userId must fail");
            assertEquals("User ID required.", r.get("message"));
        }
        @Test @DisplayName("TC-USER-WB-009: Admin deletes user successfully")
        void t009() {
            var r = userDelete(true, "user123");
            assertTrue((Boolean)r.get("success"), "Delete must pass");
            assertEquals("User deleted.", r.get("message"));
        }
        @Test @DisplayName("TC-USER-WB-010: Non-admin cannot delete user → 403")
        void t010() {
            var r = userDelete(false, "user123");
            assertFalse((Boolean)r.get("success"), "Non-admin delete must fail");
            assertEquals(403, r.get("status"));
        }
    }
