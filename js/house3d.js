...
//  Shows a configurable interior with sofa, bed, table, etc.
// ═══════════════════════════════════════════════════════════════
function initFurniture3D(canvasId, cfg) {
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;

  // Dispose existing scene if present
  ...
  const fw = 8, fd = 7, fh = 3.5; // room width, depth, height
  ...
  items.forEach(it => {
    ...
    // Extra geometry per furniture type
    if (it.key === 'bed') {
    ...
    if (it.key === 'tv') {
      fbox(1.3, 0.80, 0.06, 0x111111, it.x, it.h + 0.7, it.z);        // screen
    }
    if (it.key === 'plant') {
